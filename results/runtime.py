...
def backbone_mask(qr, qn, std_backbone, std_aminoacids):
    # backbone atom mask
    m_std_bb = pt.from_numpy(np.isin(sp.std_names, std_backbone)).to(qn.device)
    m_bb = pt.any(qn[:,:-1][:,m_std_bb] > 0.5, dim=1)

    # amino-acids or dna/rna residues
    m_std_aa = pt.from_numpy(np.isin(sp.std_resnames, std_aminoacids)).to(qr.device)
...
def chain_masks(structure):
    _, Mr, Mc = sp.encode_structure(sp.chain_name_to_index(structure))
...
class SequenceModel():
    ...
    def __call__(self, structure, m_known=None, n_skip=1):
        ...
        if self.module.config_data['virt_cb']:
            ...
            m = backbone_mask(qr, qn, np.concatenate([self.module.std_backbone, ['CB']]), self.module.std_aminoacids)
        else:
            # extract backbone mask
            m = backbone_mask(qr, qn, self.module.std_backbone, self.module.std_aminoacids)
        ...
        return structure, pt.stack(P).squeeze(), y.cpu(), M.cpu()
